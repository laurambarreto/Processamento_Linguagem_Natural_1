...
print(f"\nDados Inicialmente:", X["classe"].value_counts())
...
# Contagem de amostras por classe nos dados de treino no conjunto de validação
print(f"\nDados de treino no conjunto de validação:", train_val_df["classe"].value_counts())
...
# Primeiro, aplicar SMOTE para as classes com menos de 1000
...
# Aplicar o pipeline
X_train_bal, y_train_bal = pipeline.fit_resample(X_train_num, y_train_num)

## -- BALANCEAMENTO NOS DADOS DE TREINO NO CONJUNTO DE VALIDAÇÃO -- ##
# Definir o número alvo de amostras por classe
target_n = 1000
...
# Aplicar o pipeline
X_train_val_bal, y_train_val_bal = pipeline.fit_resample(X_train_val_num, y_train_val_num)

# Verificar o resultado
print("\nDistribuição das classes após SMOTE + undersampling:")
print(pd.Series(y_train_val_bal).value_counts())
...
X_train_num_norm = scaler.transform (X_train_bal)

# Normalização de dados (treino no conjunto de validação)
X_train_val_num_norm = scaler.transform (X_train_val_bal)
...
def KNN_optuna (trial):
    ...
    KNN.fit (X_train_val_num_norm, y_train_val_bal)

    y_pred = KNN.predict (X_val_num_norm)
...
def KNN_modelo ():
    ...
    KNN.fit (X_train_num_norm, y_train_bal)
    time_end = time.time()
    print(f"Tempo de treino do modelo KNN: {time_end - time_start:.2f}")

    y_pred = KNN.predict (X_test_num_norm)
...
def regressao_logistica_optuna (trial):
    ...
    log_reg.fit (X_train_val_num_norm, y_train_val_bal)

    y_pred = log_reg.predict (X_val_num_norm)
...
def regressao_logistica ():
    ...
    # Medir tempo de treino
    time_start = time.time()
    log_reg.fit (X_train_num_norm, y_train_bal)
    time_end = time.time()
    print(f"Tempo de treino do modelo Regressão Logística: {time_end - time_start:.2f}")

    y_pred = log_reg.predict (X_test_num_norm)
    return y_pred
...
def naive_bayes_optuna (trial):
    ...
    nb.fit (X_train_val_num_norm, y_train_val_bal)

    y_pred = nb.predict (X_val_num_norm)
    score = f1_score (y_val_num, y_pred, average = 'macro')
    return score
...
def naive_bayes_modelo ():
    ...
    # Medir tempo de treino
    time_start = time.time()
    nb.fit(X_train_num_norm, y_train_bal)
    time_end = time.time()
    print(f"Tempo de treino do modelo Naive Bayes: {time_end - time_start:.2f}")

    y_pred = nb.predict(X_test_num_norm)
    return y_pred
...
def random_forest_optuna (trial):
    ...
    rf.fit(X_train_val_num_norm, y_train_val_bal)

    y_pred = rf.predict(X_val_num_norm)
    score = f1_score (y_val_num, y_pred, average = 'macro')
    return score
...
def rede_neuronal_optuna (trial):
    ...
    rnn.fit (X_train_val_num_norm, y_train_val_bal)

    y_pred = rnn.predict (X_val_num_norm)
    score = f1_score (y_val_num, y_pred, average = 'macro')
    return score
...
def rede_neuronal_modelo ():
    ...
    # Medir tempo de treino
    time_start = time.time()
    rnn.fit (X_train_num_norm, y_train_bal)
    time_end = time.time()
    print(f"Tempo de treino do modelo Rede Neuronal: {time_end - time_start:.2f}")

    y_pred = rnn.predict (X_test_num_norm)
    return y_pred
